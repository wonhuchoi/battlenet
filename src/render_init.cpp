...
{
	std::vector<vec2> abc = std::vector<vec2>();
	RenderSystem::createSprite(sprite, texture_path, shader_name, abc, false);
}
// Create a new sprite and register it with ECS
void RenderSystem::createSprite(ShadedMesh& sprite, std::string texture_path, std::string shader_name, std::vector<vec2>& hull, bool withHull)
{
	if (texture_path.length() > 0) {
		if (withHull) {
			sprite.texture.load_from_file_with_hull(texture_path.c_str(), hull);
		}
		else {
...
// Create a new sprite and register it with ECS
void RenderSystem::createHull(std::vector<vec2>& hull, std::string texture_path)
{
...
void RenderSystem::initScreenTexture()
{
	...
	createSprite(screen_sprite, "", "screen");
