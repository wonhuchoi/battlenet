...
struct Texture
{
	...
	void load_from_file_with_hull(std::string path, std::vector<vec2>& hull);
...
// A wrapper that points to the HullMesh in the resource_cache
struct HullMeshRef
{
	std::vector<vec2>* reference_to_cache;
	HullMeshRef(std::vector<vec2>& mesh);
};
...
// A struct to define a bezier curve for use in non-linear motion
struct BezierCurve
{
	std::vector<vec2> controlPoints;
	// Tuneable scale coef that multiplies the magnitude of the velocity defined by the evaluation of the bezier curve.
	int motionScale;
};
