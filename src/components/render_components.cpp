...
void Texture::load_from_file_with_hull(std::string path, std::vector<vec2>& hull)
{
	stbi_uc* data;
	if (texture_cache.count(path) > 0)
		data = texture_cache[path];
	...
	stbi_uc* hulldata = data;
	int intx = 3;
	// divide by 4 because it is byte
	int intsize = sizeof(intx)/4;
	std::vector<vec2> points;
	for (int i = 0; i < size.x * size.y; i++) {

		int x = i % size.x;
		int y = i / size.y;
		int o = *(hulldata+intsize*3);
	...
	//Hull is from 0 to 1 and center is center of mesh
	algorithm::convex_hull(points);
	std::vector<vec2> scaled_points;
	for (auto p : points) {
		scaled_points.push_back(vec2((p.x - size.x/2) / size.x, (p.y - size.y / 2) / size.y));
	}
	hull = scaled_points;
...
// Returns a resource for every key, initializing with zero on the first query
std::vector<vec2>& cache_hull_resource(std::string key)
{
	static std::unordered_map<std::string, std::vector<vec2>> resource_cache;
	const auto it = resource_cache.find(key);
	if (it == resource_cache.end())
	{
		const auto it_succeeded = resource_cache.emplace(key, std::vector<vec2>());
		assert(it_succeeded.second);
		return it_succeeded.first->second;
	}
	return it->second;
...
HullMeshRef::HullMeshRef(std::vector<vec2>& mesh) :
	reference_to_cache(&mesh)
{};